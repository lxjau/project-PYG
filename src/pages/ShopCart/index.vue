        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input
              ...
              @change="updataCartById(cart, $event)"
            />
          </li>
...
export default {
  ...
  methods: {
    ...
    // 修改商品选中状态
    async updataCartById(cart, event) {
      let checked = event.target.checked ? 1 : 0;
      try {
        await this.$store.dispatch("updataCartById", {
          skuId: cart.skuId,
          isChecked: checked,
        });
        this.getData();
    ...
    async changeAllChecked(event) {
      try {
        await this.$store.dispatch("changeAllChecked", event);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
